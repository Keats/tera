...
use errors::{TeraResult2, TeraError2};
...
impl_rdp! {
    ...
    process! {
        ...
        _template(&self) -> TeraResult2<LinkedList<Node>> {
        ...
        _content(&self) -> TeraResult2<Option<Node>> {
            (&head: text) => {
            ...
            (_: block_tag, &name: identifier, body: _template(), _: endblock_tag, &end_name: identifier) => {
                if name != end_name {
                    ...
                    return Err(
                        TeraError2::MismatchingEndBlock(
        ...
        _elifs(&self) -> TeraResult2<LinkedList<Node>> {
        ...
        _expression(&self) -> TeraResult2<Node> {
...
pub fn parse(input: &str) -> TeraResult2<Node> {
    let mut parser = Rdp::new(StringInput::new(input));

    if !parser.template() {
        let (_, pos) = parser.expected();
        let (line_no, col_no) = parser.input().line_col(pos);
        return Err(TeraError2::InvalidSyntax(line_no, col_no));
    }
    ...
    for token in parser.queue() {
        if token.rule == Rule::op_wrong_and {
            ...
            return Err(
                TeraError2::DeprecatedSyntax(line_no, col_no, "Use `and` instead of `&&`".to_string())
            );
        }
        if token.rule == Rule::op_wrong_or {
            let (line_no, col_no) = parser.input().line_col(token.start);
            return Err(
                TeraError2::DeprecatedSyntax(line_no, col_no, "Use `or` instead of `||`".to_string())
            );
        }
    }
...
mod tests {
    ...
    fn test_ast_error_old_and() {
        ...
        assert_eq!(
            parsed_ast.err().unwrap(),
            TeraError2::DeprecatedSyntax(1, 9, "Use `and` instead of `&&`".to_string())
        );
    }
    ...
    fn test_ast_error_old_or() {
        ...
        assert_eq!(
            parsed_ast.err().unwrap(),
            TeraError2::DeprecatedSyntax(1, 9, "Use `or` instead of `||`".to_string())
        );
    }
    ...
    fn test_ast_error_mismatch_endblock_name() {
        ...
        assert_eq!(
            parsed_ast.err().unwrap(),
            TeraError2::MismatchingEndBlock(1, 33, "hey".to_string(), "ho".to_string())
        );
    }
