...
pub fn sort(value: Value, args: HashMap<String, Value>) -> Result<Value> {
    ...
    let attribute = try_get_value!(
        "sort",
        "attribute",
        String,
        args.get("attribute").unwrap_or(&"".into())
    );
...
pub fn slice(value: Value, args: HashMap<String, Value>) -> Result<Value> {
    ...
    let start = try_get_value!(
        "slice",
        "start",
        f64,
        args.get("start").unwrap_or(&0.0.into())
    ) as usize;
    // Not an error, but returns an empty Vec
    if start > arr.len() {
        return Ok(Vec::<Value>::new().into());
    }
    let mut end = try_get_value!(
        "slice",
        "end",
        f64,
        args.get("end").unwrap_or(&arr.len().into())
    ) as usize;
    if end > arr.len() {
        end = arr.len();
    }
